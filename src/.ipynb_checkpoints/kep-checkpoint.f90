...
subroutine solve_kepler_grad(t, np, t0p, ep, ap, r, cosf, sinf, &
                           r_n, r_t0, r_e, r_a, &
    ...
    integer :: j, i
    real*8 :: n, t0, ecc, a, tol, x, err
    real*8, dimension(j) :: t, M, E, sE, cE, tanf
    real*8, dimension(j), intent(out) :: r, cosf, sinf
    ...
    if (ecc .lt. 1.d-5) then
        ...
        cosf_t0 = cosf_r * r_t0
        cosf_e = cosf_r * r_e + 1.d0 / y - a * x / (r * y) - 2 * a / r
        cosf_n = cosf_r * r_n
        
        tanf = sinf / cosf
        sinf_r = - tanf * cosf_r
        sinf_a = - tanf * cosf_a
        sinf_t0 = - tanf * cosf_t0
        sinf_e = - tanf * cosf_e
        sinf_n = - tanf * cosf_n
    end if
end
...
subroutine coords_grad(t, ms, t0p, ep, Pp, Op, wp, ip, mp, &
    &t0m, em, Pm, wm, Om, im, mm, j, &
    &xp, yp, zp, xm, ym, zm, dxp, dyp, dzp, dxm, dym, dzm) bind(C, name="coords")
    ...
    real*8, dimension(j) :: cosfw_wp, cosfw_np, cosfw_Pp, cosfw_t0p, cosfw_ep, cosfw_ap, cosfw_Pp
    real*8, dimension(j) :: cosfw_wm, cosfw_nm, cosfw_Pm, cosfw_t0m, cosfw_em, cosfw_am, cosfw_Pm
    real*8, dimension(j) :: sinfw_wp, sinfw_np, sinfw_Pp, sinfw_t0p, sinfw_ep, sinfw_ap, sinfw_Pp
    real*8, dimension(j) :: sinfw_wm, sinfw_nm, sinfw_Pm, sinfw_t0m, sinfw_em, sinfw_am, sinfw_Pm
    ...
    xbc_ip = - r * (comegap * cosfw - somegap * sinfw * cip_ip)
    ...
    ybc_ip = - r * (somegap * cosfw + comegap * sinfw * cip_ip)
    ...
    call solve_kepler(t, nm, t0m, em, am, r, cosf, sinf, &
                           r_n, r_t0, r_e, r_a, &
                           cosf_n, cosf_t0, cosf_e, cosf_a, 
                           sinf_n, sinf_t0, sinf_e, sinf_a, j)
    
    cosfw = cwm * cosf - swm * sinf
    cosfw_wm = cwp_wm * cosf - swp_wm * sinf
    ...
    cosfw_Pm = cosfw_am * am_Pm
    
    sinfw = swm * cosf + sinf * cwm
    sinfw_wm = swp_wm * cosf + sinf * cwp_wm
    ...
    x_Om = - r * (comegap_Om * cosfw - somegap_Om * sinfw * cim)
    x_im = - r * (comegam * cosfw - somegam * sinfw * cip_im)
    ...
    y_Om = - r * (somegap_Om * cosfw + comegap_Om * sinfw * cim)
    y_im = - r * (somegam * cosfw + comegam * sinfw * cip_im)
...
subroutine input_coords_grad(t, ms, t0p, ep, Pp, Op, wp, ip, mp, &
    ...
    real*8, dimension(15, j) :: dxp, dyp, dzp, dxm, dym, dzm
    integer (c_int), bind(C) :: j
    real (c_double), bind(C) :: ms, t0p, ep, Pp, Op, wp, ip, mp 
    real (c_double), bind(C) :: t0m, em, Pm, Om, wm, im, mm
    real (c_double), bind(C), dimension(j) :: t
    ...
    do i=1,15,1
        ...
        dbpm(i, :) = dpm_xm * dxm(i, :) + dpm_ym * dym(i, :) &
                   + dpm_xp * dxp(i, :) + dpm_yp * dyp(i, :)
